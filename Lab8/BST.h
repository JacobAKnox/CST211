...
class BST {
 private:
  ...
  TNode<T>* Insert(const T& data, TNode<T>* node, int height);
  TNode<T>* Delete(const T& data, TNode<T>* node);
  void Purge(TNode<T>* node);

  void InOrder(Vistor v, TNode<T>* node) const;
  void PreOrder(Vistor v, TNode<T>* node) const;
  void PostOrder(Vistor v, TNode<T>* node) const;
  void BreadthFirst(Vistor v, TNode<T>* node, int level) const;

  TNode<T>* CopyHelper(TNode<T>* node) const;
  int CalcHeight(TNode<T>* node) const;

  bool Contains(const T& data, TNode<T>* node) const;
...
template <typename T>
TNode<T>* BST<T>::Insert(const T& data, TNode<T>* node, int height) {
  if (node == nullptr) {
...
template <typename T>
TNode<T>* BST<T>::Delete(const T& data, TNode<T>* node) {
  if (node == nullptr) {
...
template <typename T>
void BST<T>::InOrder(Vistor v, TNode<T>* node) const {
  if (node != nullptr) {
...
template <typename T>
void BST<T>::PreOrder(Vistor v, TNode<T>* node) const {
  if (node != nullptr) {
...
template <typename T>
void BST<T>::PostOrder(Vistor v, TNode<T>* node) const {
  if (node != nullptr) {
...
template <typename T>
void BST<T>::BreadthFirst(Vistor v, TNode<T>* node, int level) const {
  if (node == nullptr) {
...
template <typename T>
int BST<T>::CalcHeight(TNode<T>* node) const {
  if (node == nullptr) {
...
template <typename T>
TNode<T>* BST<T>::GetRoot() const {  //
  // return the root pointer
  return root;
}

template <typename T>
TNode<T>* BST<T>::CopyHelper(TNode<T>* node) const {
  if (node == nullptr) {
    // if node is null, return null
    return nullptr;
  }
  // create a new node with the node value
  TNode<T>* new_node = new TNode<T>(node->Value());
...
template <typename T>
bool BST<T>::Contains(const T& data, TNode<T>* node) const {
  if (node == nullptr) {
